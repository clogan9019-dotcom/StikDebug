...
struct HomeView: View {
    ...
    private func startJITInBackground(bundleID: String? = nil, pid: Int? = nil, scriptData: Data? = nil, scriptName: String? = nil, triggeredByURLScheme: Bool = false, displayName: String? = nil) {
        ...
        DispatchQueue.global(qos: .background).async {
            ...
            if KernelOffsets.useKernelExploit {
                let semaphore = DispatchSemaphore(value: 0)
                var exploitSuccess = false
                var exploitError: Error?

                KernelExploitJIT.shared.enableJIT(
                    logger: { message in LogManager.shared.addInfoLog(message) },
                ...
                if exploitSuccess {
                    if let bundleID {
                        _ = JITEnableContext.shared.launchAppWithoutDebug(bundleID, logger: logger)
                    }
                    DispatchQueue.main.async {
                        LogManager.shared.addInfoLog("Kernel exploit JIT enabled for \(bundleID ?? String(pid ?? 0))")
                        if doAutoQuitAfterEnablingJIT { exit(0) }
                    }
                } else {
                    lastDebugMessage = exploitError?.localizedDescription ?? "Kernel exploit JIT failed."
                }
                finishProcessing(exploitSuccess, exploitSuccess ? nil : lastDebugMessage)
                return
            }
